...
class ClientRequestController extends Controller
{
    public function sendRequest(Request $request) {
        ...
        // return redirect('/client/request')->with('success', 'Success sending request');
        return back()->with('success', 'Success sending request');
    }
    ...
    public function stockOut(Request $request) {
        ...
        $notifId = '';
        do {
            $notifId = 'STO-' . now()->format('Ymd') . '-' . Str::upper(Str::random(12));
        } while (Notifications::where('notification_id', $notifId)->exists());
